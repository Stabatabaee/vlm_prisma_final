...
FUSION_ALIASES = compile_aliases([
    ...
    (r"\bencoder[-\s]?decoder[-\s]?attention\b", "cross-attention"),
    (r"\bco[-\s]?attention\b", "co-attention"),
    (r"\bself[-\s]?attention\b", "self-attention"),
    (r"\bmulti[-\s]?modal\b|\bcross[-\s]?modal\b|\bvision[-\s]?language attention\b", "cross-attention"),
    (r"\bcondition(ed|ing)?\b", "conditioned"),
    (r"\bconcat(enation)?\b|\bprojection\s*\+\s*concat\b", "concatenation"),
    (r"\blate[-\s]?fusion\b", "late fusion"),
    (r"\bearly[-\s]?fusion\b", "early fusion"),
    (r"\bgated\b|\bgating\b", "gated"),
    (r"\bfilm\b", "FiLM"),
    (r"\bmlp[-\s]?fusion\b|\blinear[-\s]?fusion\b", "concatenation"),
])

# ------- Objectives (expanded) -------
OBJECTIVE_ALIASES = compile_aliases([
    (r"\bitm\b|image[-\s]?text matching|matching loss", "ITM"),
    (r"\b(itc|clip)\b|image[-\s]?text contrast(ive)?\b|contrastive|infonce", "contrastive"),
    (r"\bmlm\b|masked language model(ing)?\b|language modeling", "MLM"),
    (r"\bmim\b|masked image model(ing)?\b", "MIM"),
    (r"\bmrm\b|masked region model(ing)?\b", "MRM"),
    (r"\bcaption(ing)?\b|caption loss", "captioning"),
    (r"\bcoverage\b", "coverage"),
    (r"\bcross[-\s]?entropy\b|\b(ce|xe)\b|\bclassification loss\b", "cross-entropy"),
    (r"\bnext sentence prediction\b|\bnsp\b", "NSP"),
    (r"\breinforcement learning\b|\brl\b", "RL"),
    (r"\bretrieval loss\b", "ITM"),
    (r"\btriplet loss\b", "contrastive"),
    (r"\bbce\b|binary cross[-\s]?entropy", "cross-entropy"),
])
...
FAMILY_ALIASES = compile_aliases([
    ...
    (r"\bsimvlm\b", "SimVLM"),
    (r"\bofa\b", "OFA"),
    # Treat CLIP as a family candidate when needed for Model preference
    (r"\bclip\b", "CLIP"),
])
...
def normalize_fusion(cell: str) -> str:
    ...
    Keep the canonical label set small to avoid value explosion;
    availability is what matters downstream.
    """
    toks = split_multi(cell)
...
def normalize_family(cell: str) -> str:
    ...
    if FAMILY_ONTOLOGY == "coarse":
        # Very light conversion: map any named VLMs to 'Transformer' if present
        if fam:
            return "Transformer"
    return fam
...
def backfill_objectives_and_fusion(row: Dict[str, str]) -> Tuple[bool, bool]:
    ...
    # Fusion: if missing and this looks like a VLM, assume cross-attention
...
def derive_family_if_empty_from_model(row: Dict[str, str]) -> bool:
    ...
    Additionally, when --family-ontology=coarse and the row looks like a VLM,
    force Family='Transformer' if still empty (familyfix behavior).
    """
    changed = False
    if is_missing(row.get("Family", "")):
        model = _norm(row.get("Model", "")).replace(" ", "")
        for k in VLM_FAMILY_KEYS:
            if k in model:
                fam = apply_alias_once(k, FAMILY_ALIASES)
                # If ontology is coarse, normalize_family will later convert to 'Transformer'
                row["Family"] = fam
                changed = True
                break

    # familyfix: coarse ontology + VLM rows should not stay empty
    if FAMILY_ONTOLOGY == "coarse" and is_missing(row.get("Family", "")) and looks_like_vlm(row):
        row["Family"] = "Transformer"
        changed = True
