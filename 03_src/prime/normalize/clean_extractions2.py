...
# -----------------------
# FAMILY improvements only
# -----------------------
FAMILY_ALIASES = compile_aliases([
    # Canonical VLM families (+ common typos/variants)
    (r"\balbef\b", "ALBEF"),
    (r"\bblip[-\s]?2\b|blip v?2\b|blip2\b", "BLIP-2"),
    (r"\bblip\b", "BLIP"),
    (r"\bflamingo\b", "Flamingo"),
    (r"\bllava[-\s]?med\b|\bllava\s*med\b", "LLaVA-Med"),
    (r"\bllava\b", "LLaVA"),
    (r"\bmini[-\s]?gpt[-\s]?4\b|minigpt[-\s]?4\b", "MiniGPT-4"),
    (r"\binstruct[-\s]?blip\b", "InstructBLIP"),
    (r"\botter\b", "Otter"),
    (r"\bx[-\s]?ray[-\s]?gpt\b|xraygpt", "XrayGPT"),
    (r"\bpali[-\s]?x\b", "PaLI-X"),
    ...
    # Optional domain "families" sometimes used like families in papers (helps availability)
    (r"\bchexnet\b", "CheXNet"),
...
# (order unchanged except we allow more to be matched)
PRIMARY_FAMILY_ORDER = [
    "LLaVA", "LLaVA-Med", "BLIP-2", "InstructBLIP", "BLIP", "ALBEF",
    "Flamingo", "MiniGPT-4", "Otter", "XrayGPT", "PaLI-X", "PaLI",
    "Kosmos-1", "SimVLM", "OFA", "CLIP",
    # domain-labeled families as fallback
    "CheXNet", "CheXpert", "CXR-IRGen", "TrMRG",
]

# Expand keys used to mine from Model/row blobs
VLM_FAMILY_KEYS = [
    "albef", "blip-2", "blip2", "blip", "flamingo", "llava-med", "llava",
    "minigpt-4", "instructblip", "otter", "xraygpt", "pali-x", "pali",
    "kosmos-1", "simvlm", "ofa", "clip",
    # helpful domain families
    "chexnet", "chexpert", "cxr-irgen", "trmrg",
]

# Family preference order for "primary model"
# (kept above, unchanged semantics)
...
VLM_FAMILY_KEYS = VLM_FAMILY_KEYS  # keep our expanded list
...
def backfill_objectives_and_fusion(row: Dict[str, str]) -> Tuple[bool, bool]:
    """Return (obj_filled, fusion_filled). Uses 'missing' semantics incl. 'Not reported'."""
    obj_filled = False
    fus_filled = False
    ...
    if is_missing(row.get("Objectives", "")):
        ...
        for k, v in OBJ_MAP.items():
            if k in model_fam_blob:
                ...
                obj_filled = True
                break
        # generic default for VLMs if still missing
...
# NEW: mine Family from other text if still empty (File/Title/Abstract/Notes…)
def derive_family_if_empty_from_text(row: Dict[str, str]) -> bool:
    if not is_missing(row.get("Family", "")):
        return False
    # Build a lightweight blob from high-signal columns
    parts = []
    for k in ["File", "Title", "Abstract", "Notes", "Method", "Approach", "System"]:
        v = row.get(k, "")
        if v:
            parts.append(str(v))
    blob = " | ".join(parts)
    cand = scan_text(blob, FAMILY_ALIASES)
    if cand:
        row["Family"] = cand
        return True
    return False
...
def clean_row(row: Dict[str, str]) -> Dict[str, str]:
    ...
    # (Family improvements only: first from Model, then from surrounding text if still empty)
    derive_family_if_empty_from_model(out)
    derive_family_if_empty_from_text(out)
