...
Coverage definition:
  For each dataset: fraction of non-"Not reported" cells over all data cells
  (excluding the 'File' column). Also reports per-column coverage and
  per-row (per-paper) coverage averages.

Example:
  python eval_coverage_v14.py \
    --inputs \
      "MMR strict:filled_papers_vlm_v14_mmr_strict.csv" \
      "SIM strict:filled_papers_vlm_v14_sim_strict.csv" \
      "MMR lenient:filled_papers_vlm_v14_mmr_lenient.csv" \
      "SIM lenient:filled_papers_vlm_v14_sim_lenient.csv" \
    --out-csv coverage_summary_v14.csv \
    --out-md coverage_report_v14.md
"""
...
def summarize(rows):
    """Return (overall_cov, per_col_cov, per_row_cov_mean) and metadata."""
    if not rows:
        return 0.0, {}, 0.0, [], []
...
def main():
    ...
    # If we have both MMR and SIM in labels, show deltas (MMR - SIM) for strict and lenient
    def find(lbls, key):
