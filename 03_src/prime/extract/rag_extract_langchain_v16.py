...
"""
rag_extract_langchain_v16.py

V16 notes:
- Keeps v15 behavior & CLI surface, PLUS:
  * --no-append-full to ablate adding the full snippet to the retrieved context
  * --ctx-budget-toks to cap context size (approx chars ~= toks*3.5)
  * Expanded MODEL_CANON to canonicalize common VLM families/aliases
- Preserves legacy LangChain imports (fits your current env)
- Uses HF pipeline with device_map="auto" (no explicit device arg)
...
DB_DIR = "chroma_db"
# Default character budget; can be overridden by --ctx-budget-toks
CTX_CHAR_LIMIT = 7000  # was 18000
...
MODEL_CANON = {
    ...
    r"\botter\b": "Otter",
    r"\bradfm\b": "RadFM",
    r"\bmedflamingo\b": "Med-Flamingo",
    r"\bradiology[- ]?gpt\b": "Radiology-GPT",
    r"\bllama[- ]?adapter\b": "LLaMA-Adapter",
    r"\bpaligemma\b": "PaLI-Gemma",
    r"\bpali\b": "PaLI",
    r"\bqwen[- ]?vl\b": "Qwen-VL",
    r"\bidefics\b": "IDEFICS",
    r"\bkosmos[- ]?2\b": "KOSMOS-2",
    r"\bopenflamingo\b": "OpenFlamingo",
}
...
def build_context(
    ...
    if not docs and retrieval == "mmr" and vectordb:
        # fallback to similarity if mmr fails
...
# --------------- Main ---------------
def main():
    ap=argparse.ArgumentParser(description="Extract PRISMA/VLM metadata (v16).")
    ...
    # New in v16: token budget for context; char budget ~= toks*3.5
    ap.add_argument("--ctx-budget-toks", type=int, default=None, help="Approximate token budget for context (chars ~= toks*3.5)")

    args=ap.parse_args()

    # Resolve context budget (chars)
    char_budget = CTX_CHAR_LIMIT
    if args.ctx_budget_toks is not None and args.ctx_budget_toks > 0:
        # Empirical: ~3.5 chars/token for English prose
        char_budget = int(args.ctx_budget_toks * 3.5)
    ...
    for base in bases:
        context = build_context(
            vectordb, snippets_dir, base, args.k, args.retrieval, args.debug, args.no_append_full, char_budget
        ) if vectordb else (
            "" if args.no_append_full else (read_full_snippet(snippets_dir, base) or "")
        )
        if args.debug and not vectordb and context:
            debugln(f"[DEBUG] {base}: no vectorstore; context chars={len(context)}", args.debug)
