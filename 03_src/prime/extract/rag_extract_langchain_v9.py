...
"""
V9 — VLM-centric extractor for systematic review tables (hardened).

Fixes in this version:
- Accept list/scalar for fields like `modality` (no `.lower()` on list crash)
- Coerce booleans from strings ("true"/"false") reliably
- Handle list-or-string for encoders/decoders/metrics/objectives, etc.
- Prefer retriever.invoke() with safe fallback to get_relevant_documents()
- Robust imports for Chroma & HuggingFaceEmbeddings across LangChain versions
"""
...
try:
    ...
    try:
        ...
        try:
            ...
            print("[FATAL] Could not import Chroma. Install `langchain-chroma` or `langchain-community`.", file=sys.stderr)
            raise e
...
try:
    ...
    try:
        ...
        try:
            # legacy (deprecated warnings OK)
            from langchain.embeddings import HuggingFaceEmbeddings as SentenceTransformerEmbeddings
        except Exception as e:
            print("[FATAL] Could not import HuggingFaceEmbeddings. Install `langchain-huggingface` or `langchain-community`.", file=sys.stderr)
            raise e
...
# -----------------------------
# Prompt / Extractor Pack (same as v9)
# -----------------------------
PREAMBLE = (
    ...
    "- Do NOT guess, do NOT add citations outside `evidence`.\n"
    "- Keep `evidence` short (a key phrase or line refs).\n"
    "- Use canonical names where possible (e.g., ViT-B/16, CLIP-ViT-L/14, Swin-T, ResNet50, BERT, T5, GPT-2, LLaMA-2/3).\n"
    "- Prefer the actual generation decoder for language_decoder (not encoders used for retrieval).\n"
)
...
PROMPTS: Dict[str, Dict[str, str]] = {
    ...
    "metrics_key_for_vlm": {
        "schema": json.dumps({
            "metrics": ["CIDEr","BERTScore","ROUGE-L","BLEU-4","METEOR","CheXbert-F1","RadGraph-F1","RadEntity/Relation F1","Clinical Efficacy","Not reported"],
...
def load_local_llm(model_name: str, max_new_tokens: int = 512, temperature: float = 0.0):
    if not HF_AVAILABLE:
        die("Transformers not available. Install transformers/torch to use local LLM.")
...
def llm_json(pipe, prompt: str, debug: bool = False) -> Dict[str, Any]:
    ...
    if USE_OPENAI and not isinstance(pipe, (TextGenerationPipeline, type(None))):
        ...
        if hasattr(client, "ChatCompletion"):
            ...
            die("OpenAI SDK version mismatch—set a local model or install openai==0.28.*")
    else:
...
def first_scalar(x: Any) -> str:
    """Return a reasonable scalar string from list/str/other."""
    if x is None:
        return "Not reported"
...
def to_list(x: Any) -> List[str]:
    """Normalize to list of strings (or ['Not reported'])."""
    if x is None:
        return ["Not reported"]
    if isinstance(x, list):
        vals = [norm_space(str(i)) for i in x if str(i).strip()]
...
# -----------------------------
# Regex backend (deterministic)
# -----------------------------
DATASET_PAT = re.compile(r"\b(MIMIC[- ]?CXR|IU ?X[- ]?Ray|Open[- ]?i|Open[- ]?I|CheXpert|RSNA|PadChest|VinDr|BRAX)\b", re.I)
...
def regex_extract(all_text: str) -> Dict[str, Any]:
    datasets = sorted(set(m.group(1).replace(" ", "").replace("-", "-") for m in DATASET_PAT.finditer(all_text)))
...
def merge_results(acc: Dict[str, Any], section_key: str, obj: Dict[str, Any]):
    # Each section normalized via helpers
...
def main():
    ...
    ap.add_argument("--snippets-dir", type=str, required=True, help="Directory with .txt snippets (already chunked or full).")
    ap.add_argument("--out-md", type=str, default="filled_papers_vlm_v9.md")
    ap.add_argument("--out-csv", type=str, default="filled_papers_vlm_v9.csv")

    # Retrieval / RAG
    ap.add_argument("--k", type=int, default=10, help="chunks per prompt")
    ap.add_argument("--no-mmr", action="store_true", help="disable MMR (use plain similarity)")

    # Backend
    ap.add_argument("--backend", choices=["regex","llm"], default="regex")
    ap.add_argument("--local-model", type=str, default="", help="HF model (e.g., meta-llama/Meta-Llama-3-8B-Instruct)")
    ...
    if args.backend == "llm":
        if args.local_model:
            pipe = load_local_llm(args.local_model, max_new_tokens=800, temperature=args.temperature)
        elif USE_OPENAI:
            pipe = "openai"
        else:
    ...
    for doc_id in paper_ids:
        ...
        if args.backend == "regex":
            ...
            # final list/scalar normalization
