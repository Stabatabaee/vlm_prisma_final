#!/usr/bin/env python3
"""
rag_extract_langchain_v5.py

Key fixes vs v4:
- Much stricter output cleaning: keep ONLY a single short phrase; strip any commentary.
- For HF local LLM, use return_full_text=False and truncate at first newline/#/```/---/Answer.
- Extra guards: if output contains "you are", "PRISMA", "context", "correct answer", etc. -> Not reported.
- Datasets still derived via lexical sweep from full text when present.
- Per-paper retrieval with doc_id filter; similarity search; distinct chunk de-dup.

Usage (local LLaMA):
  unset OPENAI_API_KEY
  CUDA_VISIBLE_DEVICES=0 python rag_extract_langchain_v5.py \
    --snippets-dir snippets \
    --local-model meta-llama/Meta-Llama-3-8B-Instruct \
    --out-md filled_papers_llama_v5.md \
    --out-csv filled_papers_llama_v5.csv
"""
...
INSTRUCTIONS = (
    ...
    "Return ONLY a SINGLE SHORT PHRASE for the requested field using ONLY the provided context.\n"
    "No explanations, no labels, no punctuation beyond commas and hyphens, no code, no examples.\n"
    "If the field is not present, return exactly: Not reported\n"
)

BAD_MARKERS = [
    "you are", "correct answer", "context", "please note", "prisma", "pdf", "table",
    "code", "example", "```", "###", "####", "—", "Answer:", "Output:"
]

# ---------------------- small utils ----------------------
def _first_phrase(s: str) -> str:
    """Take the first phrase: cut at newline, '#', '```', '---', 'Answer:', or sentence end."""
    s = s.strip()
    for cut in ["\n", "#", "```", "---", "Answer:", "Output:"]:
        if cut in s:
            s = s.split(cut, 1)[0].strip()
    # Cut at first sentence end if long
    m = re.search(r"[.!?]", s)
    if m and m.start() > 0:
        s = s[:m.start()].strip()
    # Collapse spaces
...
def clean_one_phrase(text: str, field: str) -> str:
    if not text:
        return "Not reported"
    t = _first_phrase(text)

    # Remove any obvious field labels e.g., "Title: ..."
    t = re.sub(r"^(title|authors|modality|datasets|model name|vision encoder|language decoder|fusion strategy)\s*:\s*", "", t, flags=re.I)
    # If any bad markers survive, discard
    t_low = t.lower()
    if any(b in t_low for b in BAD_MARKERS):
        return "Not reported"
    # Strip quotes/backticks/markdown
    t = t.strip(" `*_'\"")
    # Safety caps
    if len(t) > 80:
        t = t[:80].strip()
    # Empty after cleaning?
    if not t or t.lower() == "answer" or t.lower().startswith("from the context"):
        return "Not reported"

    # Field-specific normalization
    if field in CLAMP_FIELDS:
        # Keep only simple tokens and common separators
        t = re.sub(r"[^A-Za-z0-9+/\-&,; ]", "", t).strip()
    if field == "Authors":
        # Reduce to "A., B., C." style phrase-like output
        t = re.sub(r"\s{2,}", " ", t)
    if field == "Title":
        # Title should not end with trailing punctuation spam
        t = t.rstrip(" .:-")
...
def call_llm(llm, prompt: str, field: str, tok: Optional[AutoTokenizer]) -> str:
    """ChatOpenAI or HF pipeline -> one-line answer."""
    if hasattr(llm, "invoke"):  # OpenAI path
        out = llm.invoke(prompt).content
        return clean_one_phrase(out, field)
    ...
    gen = llm(
        ...
        max_new_tokens=64,
        do_sample=False,
        return_full_text=False,   # critical: only completion
    )
    text = gen[0]["generated_text"]
    if "Answer:" in text:
        text = text.split("Answer:", 1)[-1]
    return clean_one_phrase(text, field)
...
# ---------------------- Core extraction ----------------------
def make_prompt(field: str, pid: str, context_text: str) -> str:
    ...
    prompt = (
        ...
        f"\"\"\"\n{context_text}\n\"\"\"\n\n"
        "Return only the phrase. Do not include the word 'Answer'.\n"
        "Answer:"
    )
    return prompt

def ask_field(llm, tok, field: str, pid: str, context_text: str) -> str:
    prompt = make_prompt(field, pid, context_text)
    return call_llm(llm, prompt, field, tok)
...
def parse_args():
    ...
    ap.add_argument("--out-md", default="filled_papers_llama_v5.md")
    ap.add_argument("--out-csv", default="filled_papers_llama_v5.csv")
