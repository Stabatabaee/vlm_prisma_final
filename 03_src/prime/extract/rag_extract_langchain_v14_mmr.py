...
"""
rag_extract_langchain_v14_mmr.py
...
- Backfills: v12-style backfill for datasets & modality (and pragmatic enc/dec backfill).
- Gating: `--strict-evidence` optional (off by default).
- JSON: Robust balanced-JSON extraction with retries; regex fallback if JSON fails.
- Outputs: CSV + Markdown.

Quick usage examples are at the end of this message.
"""
...
def retrieve_context_mmr(vectordb, base: str, snippets_dir: Path, k: int, debug: bool)->str:
    ...
    texts, total = [], 0
    try:
        docs = vectordb.max_marginal_relevance_search(query=query, k=k, fetch_k=max(40,4*k), lambda_mult=0.5, filter={"doc_id": base})
    except Exception:
        docs = []
    if not docs:
        try:
...
def main():
    ap=argparse.ArgumentParser(description="Extract PRISMA/VLM metadata (v14 MMR).")
