...
"""
rag_extract_langchain_v17.py

Fast, parallel-friendly PRISMA-style VLM metadata extractor.

Changes vs v17 (quality hotfix):
- Prompt: forbid copying schema/type words into values
- Post-parse sanitization: replace schema-type placeholders with "Not reported"
- Recommend running with --strict-evidence to suppress baseline name bleed-through
"""
...
# Silence HF logs early
import transformers, torch
transformers.logging.set_verbosity_error()

# modest speedups on Ampere+
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True

# keep tokenizer single-threaded in multiproc
os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")

# ---- LangChain / Chroma (legacy imports to fit your env) ----
...
DB_DIR = "chroma_db"
DEFAULT_CTX_CHAR_LIMIT = 18000  # can be overridden by --ctx-budget-toks * 4
...
# Canonicalize model names from variants/mentions (expanded)
MODEL_CANON = {
    r"\btrmrg\b|Transformer Medical report generator": "TrMRG",
    r"\beggca[- ]?net\b": "EGGCA-Net",
    r"\bcxr[- ]?irgen\b": "CXR-IRGen",
    r"\balbef\b": "ALBEF",
    # Added families/modern VLMs
    r"\bblip[- ]?2\b": "BLIP-2",
    r"\bblip\b": "BLIP",
    r"\bllava\b": "LLaVA",
    r"\bflamingo\b": "Flamingo",
    r"\bgpt[- ]?4v(ision)?\b|\bgpt[- ]?4 vision\b|\bgpt[- ]?4[- ]?vision\b": "GPT-4V",
    r"\bminigpt[- ]?4\b": "MiniGPT-4",
    r"\botter\b": "Otter",
    r"\bllava[- ]?med\b": "LLaVA-Med",
    r"\bqwen[- ]?vl\b": "Qwen-VL",
    r"\bidefics\b": "IDEFICS",
}

# Expanded evidence (notably RAG)
EVIDENCE_PATTERNS = {
    ...
    try:
        if retrieval == "mmr":
            return vectordb.max_marginal_relevance_search(
                query=query, k=k, fetch_k=max(80, 6*k), lambda_mult=0.45, filter={"doc_id": base}
            )
        else:
            return vectordb.similarity_search(query=query, k=k, filter={"doc_id": base})
    except Exception:
        return []

def build_context(vectordb, snippets_dir: Path, base: str, k: int, retrieval: str, debug: bool, no_append_full: bool, ctx_char_limit: int)->str:
    texts, total = [], 0
    docs = retrieve(vectordb, base, k, retrieval, debug) if vectordb else []
    if not docs and retrieval == "mmr" and vectordb:
        docs = retrieve(vectordb, base, k, "similarity", debug)
    for d in docs or []:
    ...
    if debug:
        debugln(f"[DEBUG] paper={base} | retrieved={len(docs or [])} | context_chars={total} / {ctx_char_limit}", debug)
...
LLM_SYSTEM_INSTRUCTIONS = (
    ...
    "Never copy the schema/type words like 'string', 'list', or \"string or list\" into the values."
)
LLM_JSON_SCHEMA = {
...
def make_llm_prompt(context: str, filename: str)->str:
    ...
    return (
        ...
        "- Do NOT copy example/type placeholders like 'string', 'list', or \"string or list\"; if unknown, use 'Not reported'.\n"
    )
...
_PLACEHOLDER_RX = re.compile(r"^(string|list)(?:\s+or.*)?$", re.I)

def _sanitize_values(j: Dict[str,Any])->Dict[str,Any]:
    """Convert schema/type placeholders to 'Not reported' and strip whitespace."""
    fixed={}
    for k,v in j.items():
        if isinstance(v, str):
            val=v.strip()
            if _PLACEHOLDER_RX.match(val) or "Not applicable" in val:
                fixed[k]="Not reported"
            else:
                fixed[k]=val
        elif isinstance(v, list):
            nv=[]
            for el in v:
                if isinstance(el,str) and _PLACEHOLDER_RX.match(el.strip()):
                    continue
                nv.append(el)
            fixed[k]=nv if nv else "Not reported"
        else:
            fixed[k]=v
    return fixed
...
def load_local_llm(model_name: str):
    ...
    if quant_ok and quant is not None:
        mdl=AutoModelForCausalLM.from_pretrained(model_name, device_map="auto", quantization_config=quant)
    else:
    ...
    if getattr(mdl,"hf_device_map",None):
        println(f"Device map: {mdl.hf_device_map}")
    return gen, tok

def llm_generate(gen, tok, prompt:str, max_new_tokens:int)->str:
    out=gen(
        prompt,
...
def run_llm_backend(base:str, context:str, gen, tok, retries:int, debug:bool, strict:bool, max_new_tokens:int)->Tuple[Optional[Dict[str,str]], Optional[str]]:
    prompt=make_llm_prompt(context, base); attempt=0; last_err=None
    while attempt<=retries:
        attempt+=1
        try:
            raw=llm_generate(gen, tok, prompt, max_new_tokens)
            debugln(f"[DEBUG] raw LLM text for {base} (attempt {attempt}):\n{raw}\n", debug)
            data=parse_llm_json(raw)
            data=_sanitize_values(data)  # <<< sanitize placeholders
...
# --------------- Main ---------------
def main():
    ap=argparse.ArgumentParser(description="Extract PRISMA/VLM metadata (v17 hotfix).")
    ...
    ap.add_argument("--ctx-budget-toks", type=int, default=0, help="Approx token budget for context; chars ~= toks*4 (0=use default)")
    ...
    # Determine character budget from token budget (approx 4 chars/token)
    ctx_char_limit = DEFAULT_CTX_CHAR_LIMIT
    if args.ctx_budget_toks and args.ctx_budget_toks > 0:
        ctx_char_limit = max(1000, args.ctx_budget_toks * 4)
    ...
    gen=None; tok=None
    if args.backend=="llm":
        gen, tok = load_local_llm(args.local_model)
    ...
    for base in bases:
        context = build_context(
            vectordb, snippets_dir, base, args.k, args.retrieval, args.debug,
            args.no_append_full, ctx_char_limit
        ) if vectordb else ("" if args.no_append_full else (read_full_snippet(snippets_dir, base) or ""))[:ctx_char_limit]

        if args.debug and not vectordb and context:
            debugln(f"[DEBUG] {base}: no vectorstore; context chars={len(context)} / {ctx_char_limit}", args.debug)

        if args.backend=="regex" or not gen:
            rec=run_regex_backend(base, context or "", args.strict_evidence, args.debug)
            records.append(rec); println(f"✓ {base}"); continue

        rec, err=run_llm_backend(base, context or "", gen, tok, args.retries, args.debug, args.strict_evidence, args.max_new_tokens)
        if rec:
            records.append(rec); println(f"✓ {base}")
        else:
            debugln(f"[DEBUG] LLM JSON failed for {base}: {err}. Falling back to regex.", args.debug)
            rec=run_regex_backend(base, context or "", args.strict_evidence, args.debug)
            records.append(rec); println(f"✓ {base} (regex fallback)")
