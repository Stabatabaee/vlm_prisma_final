...
"""
rag_extract_langchain_v15.py

PRISMA-style VLM metadata extractor with configurable retrieval and strict gating.

Key features vs v14:
- --retrieval {mmr, similarity} switch (MMR tuned with fetch_k/lambda_mult)
- --no-append-full ablation flag (measure retrieval impact)
- Expanded strict evidence for RAG (broader real-world phrasing)
- Metric normalization (BLEU-4 -> BLEU, ROUGE-L -> ROUGE, Acc. -> Accuracy, etc.)
- v12-style backfills (datasets/modality + pragmatic enc/dec) retained
- Robust JSON block parsing; regex fallback
...
def build_context(vectordb, snippets_dir: Path, base: str, k: int, retrieval: str, debug: bool, no_append_full: bool)->str:
    texts, total = [], 0
...
# --------------- Main ---------------
def main():
    ap=argparse.ArgumentParser(description="Extract PRISMA/VLM metadata (v15).")
    ...
    records=[]
    for base in bases:
        context = build_context(vectordb, snippets_dir, base, args.k, args.retrieval, args.debug, args.no_append_full) if vectordb else ("" if args.no_append_full else (read_full_snippet(snippets_dir, base) or ""))
        if args.debug and not vectordb and context: debugln(f"[DEBUG] {base}: no vectorstore; context chars={len(context)}", args.debug)
